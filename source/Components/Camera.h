...
struct Camera
{
    ...
    void GenerateViewMatrix(Transform& transform)
    {
        ...
        projectionMatrix[0][0] = Rx;
        projectionMatrix[0][1] = Ux;
        projectionMatrix[0][2] = Fx;
        projectionMatrix[0][3] = 0;

        projectionMatrix[1][0] = Ry;
        projectionMatrix[1][1] = Uy;
        projectionMatrix[1][2] = Fy;
        projectionMatrix[1][3] = 0;

        projectionMatrix[2][0] = Rz;
        projectionMatrix[2][1] = Uz;
        projectionMatrix[2][2] = Fz;
        projectionMatrix[2][3] = 0;

        projectionMatrix[3][0] = -(Rx * Px + Ry * Py + Rz * Pz);
        projectionMatrix[3][1] = -(Ux * Px + Uy * Py + Uz * Pz);
        projectionMatrix[3][2] = -(Fx * Px + Fy * Py + Fz * Pz);
